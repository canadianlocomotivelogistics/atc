VERSION 4.00
Begin VB.Form ProgrammingEasyScreenConsistFunctions 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Automatic Train Control - Programming Mode - Easy Screen - Consist Functions"
   ClientHeight    =   3075
   ClientLeft      =   720
   ClientTop       =   5490
   ClientWidth     =   7050
   Height          =   3480
   Icon            =   "ProgrammingEasyScreenConsistFunctions.frx":0000
   Left            =   660
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   ScaleHeight     =   3075
   ...
   Begin VB.Label Label5 
      Caption         =   $"ProgrammingEasyScreenConsistFunctions.frx":0442
      Height          =   375
      Left            =   840
      TabIndex        =   24
      Top             =   120
   ...
   Begin VB.Image ImageIcon 
      Height          =   480
      Left            =   120
      Picture         =   "ProgrammingEasyScreenConsistFunctions.frx":04E8
      Top             =   120
      Width           =   480
   End
   Begin VB.Line Line2 
   ...
   End
End
Attribute VB_Name = "ProgrammingEasyScreenConsistFunctions"
Attribute VB_Creatable = False
Attribute VB_Exposed = False

Private Sub ButtonClose_Click()

Let ProgrammingDecoder!LocomotiveDecoderCVd(21) = textboxcvvalue(21).Text
Let ProgrammingDecoder!LocomotiveDecoderCVd(22) = textboxcvvalue(22).Text
' -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
' -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    For TemporaryCounter = 9 To 0 Step -1
        ...
        If TemporaryScreen <> "Unused" Then
            If TemporaryScreen = "Programming Easy Screen Consist Functions Screen" Then
            ...
            ElseIf MainScreen!menuDebugMode.Caption = "&Debug Mode is On" Then
                ...
                Let Ini.Value = "Programming Easy Screen Consist Functions Screen, Button Close, current window is not listed in the stack to remove it and hide."
            End If
    ' -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    ' -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
            ...
            ElseIf MainScreen!menuDebugMode.Caption = "&Debug Mode is On" Then
                ...
                Let Ini.Application = "Log Errors"
                Let Ini.Parameter = Date$ & " " & Time$
                Let Ini.Value = "Programming Easy Screen Consist Functions Screen, Button Close, trying to display the previous window using the screen stack, window not recognized."
            End If
' -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
' End Loop Premature
' -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
' -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    If TemporaryCounter = -1 Then
        If MainScreen!menuDebugMode.Caption = "&Debug Mode is On" Then
            ...
            Let Ini.Application = "Log Errors"
            Let Ini.Parameter = Date$ & " " & Time$
            Let Ini.Value = "Programming Easy Screen Consist Functions Screen, Button Close, stack is empty, underflow."
        End If
    End If
' -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
' End Subroutine
...
Private Sub ButtonPrint_Click()
    ...
    ProgrammingEasyScreenConsistFunctions.PrintForm
...
' -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    For TemporaryCounter = 0 To 9
        ...
        ' ---------------------------------------------------------------------------------------------------------------------------------------------------------------------
            Let Ini.Value = "Programming Easy Screen Consist Functions Screen"
            Let TemporaryCounter = 11
        End If
    ' -------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
' ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    If TemporaryCounter = 10 Then
        If MainScreen!menuDebugMode.Caption = "&Debug Mode is On" Then
            ...
            Let Ini.Application = "Log Errors"
            Let Ini.Parameter = Date$ & " " & Time$
            Let Ini.Value = "Programming Easy Screen Consist Functions Screen, Form Activate, stack is full, overflow."
        End If
    End If
...
' -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    If TemporaryBackgroundImage = "On" Then
        ...
        Else
            If MainScreen!menuDebugMode.Caption = "&Debug Mode is On" Then
                ...
                Let Ini.Application = "Log Errors"
                Let Ini.Parameter = Date$ & " " & Time$
                Let Ini.Value = "Programming Easy Screen Consist Functions Screen , Form Activate, variable error in ATC.INI file for 'Transparency' setting."
            End If
        End If
    ...
    Else
        If MainScreen!menuDebugMode.Caption = "&Debug Mode is On" Then
            ...
            Let Ini.Application = "Log Errors"
            Let Ini.Parameter = Date$ & " " & Time$
            Let Ini.Value = "Programming Easy Screen Consist Functions Screen , Form Activate, variable error in ATC.INI file for 'Background' setting."
        End If
    End If
...
' -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    Let Ini.Filename = App.Path$ & "\Atc.ini"
    Let Ini.Application = "Programming Easy Screen Consist Functions Screen"
    Let Ini.Parameter = "Top"
    Let Ini.Value = Str$(ProgrammingEasyScreenConsistFunctons.Top)
    Let Ini.Parameter = "Left"
    Let Ini.Value = Str$(ProgrammingEasyScreenConsistFunctons.Left)
    Let Ini.Parameter = "Width"
    Let Ini.Value = Str(ProgrammingEasyScreenConsistFunctons.Width)
    Let Ini.Parameter = "Height"
    Let Ini.Value = Str(ProgrammingEasyScreenConsistFunctons.Height)
...
' -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    If TemporaryBackgroundImage = "On" Then
        ...
        Else
            If MainScreen!menuDebugMode.Caption = "&Debug Mode is On" Then
                ...
                Let Ini.Application = "Log Errors"
                Let Ini.Parameter = Date$ & " " & Time$
                Let Ini.Value = "Programming Easy Screen Consist Functions Screen , Form Deactivate, variable error in ATC.INI file for 'Transparency' setting."
            End If
        End If
    ElseIf TemporaryBackgroundImage = "Off" Then
        AlphaBlend.Enabled = False
    Else
        If MainScreen!menuDebugMode.Caption = "&Debug Mode is On" Then
            ...
            Let Ini.Application = "Log Errors"
            Let Ini.Parameter = Date$ & " " & Time$
            Let Ini.Value = "Programming Easy Screen Consist Functions Screen , Form Deactivate, variable error in ATC.INI file for 'Background' setting."
        End If
    End If
...
'--------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    If Val(TemporaryValueLeft) = 0 And Val(TemporaryValueTop) = 0 Then
        ProgrammingEasyScreenConsistFunctions.Left = (Screen.Width - Width) / 2
        ProgrammingEasyScreenConsistFunctions.Top = (Screen.Height - Height) / 2
    Else
        If Val(TemporaryValueLeft) + ProgrammingEasyScreenConsistFunctions.Width > Screen.Width Then
            Let ProgrammingEasyScreenConsistFunctions.Left = Screen.Width - ProgrammingEasyScreenConsistFunctions.Width
        Else
            Let ProgrammingEasyScreenConsistFunctions.Left = Val(TemporaryValueLeft)
        End If
        If Val(TemporaryValueTop) + ProgrammingEasyScreenConsistFunctions.Height > Screen.Height Then
            Let ProgrammingEasyScreenConsistFunctions.Top = Screen.Height - ProgrammingEasyScreenConsistFunctions.Height
        Else
            Let ProgrammingEasyScreenConsistFunctions.Top = Val(TemporaryValueTop)
        End If
    End If
'--------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
' -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    If MainScreen.menuBalloonHelp.Caption = "&Balloon Help is On" Then
        ...
        Let TemporaryText1 = "This text box displays the value of configuration" & vbCrLf & "variable twenty-one, the function(s) which can be controlled" & vbCrLf & "while the decoder is consisted with another decoder." & vbCrLf & "When the 'Easy Screen for Consist Functions' is 'Closed' the" & vbCrLf & "value of the configuration varianle will be copied to the 'Programming" & vbCrLf & "for Decoder' screen."
        ...
        Let TemporaryText1 = "This button closes the current window" & vbCrLf & "and returns you to the previous screen."
...
Private Sub Form_Resize()
    ...
    If ProgrammingEasyScreenConsistFunctions.WindowState = vbMinimized Then
    ...
    ElseIf ProgrammingEasyScreenConsistFunctions.WindowState = vbNormal Then
...
'
    ...
    Let Ini.Filename = App.Path$ & "\Atc.ini"
    Let Ini.Application = "Programming Easy Screen Consist Functions"
    Let Ini.Parameter = "Top"
    Let Ini.Value = Str$(ProgrammingEasyScreenConsistFunctions.Top)
    Let Ini.Parameter = "Left"
    Let Ini.Value = Str$(ProgrammingEasyScreenConsistFunctions.Left)
    Let Ini.Parameter = "Width"
    Let Ini.Value = Str(ProgrammingEasyScreenConsistFunctions.Width)
    Let Ini.Parameter = "Height"
    Let Ini.Value = Str(ProgrammingEasyScreenConsistFunctions.Height)
