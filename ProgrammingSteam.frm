VERSION 4.00
Begin VB.Form ProgrammingSteam 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Automatic Train Control - Programming Mode - Steam Database"
   ClientHeight    =   2865
   ClientLeft      =   2160
   ClientTop       =   5115
   ClientWidth     =   6690
   Height          =   3270
   Icon            =   "ProgrammingSteam.frx":0000
   Left            =   2100
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   ScaleHeight     =   2865
   ...
   Begin VB.PictureBox Picture1 
      ...
      Height          =   480
      Left            =   120
      Picture         =   "ProgrammingSteam.frx":0442
      ScaleHeight     =   480
      ScaleWidth      =   480
      TabIndex        =   1
      TabStop         =   0   'False
   ...
   Begin VB.Label Label1 
      Caption         =   $"ProgrammingSteam.frx":0884
      Height          =   495
      Left            =   720
      TabIndex        =   3
      Top             =   120
   ...
   End
End
Attribute VB_Name = "ProgrammingSteam"
...
' -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    For TemporaryCounter = 9 To 0 Step -1
        ...
        If TemporaryScreen <> "Unused" Then
            If TemporaryScreen = "Programming Steam Screen" Then
            ...
            ElseIf MainScreen!menuDebugMode.Caption = "&Debug Mode is On" Then
                ...
                Let Ini.Value = "Programming Steam Screen, Button Close, current window is not listed in the stack to remove it and hide."
            End If
    ...
    ' -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
            ...
            ElseIf MainScreen!menuDebugMode.Caption = "&Debug Mode is On" Then
                ...
                Let Ini.Application = "Log Errors"
                Let Ini.Parameter = Date$ & " " & Time$
                Let Ini.Value = "Programming Steam Screen, Button Close, trying to display the previous window using the screen stack, window not recognized."
            End If
' -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
' End Loop Premature
' -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
' -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    If TemporaryCounter = -1 Then
        If MainScreen!menuDebugMode.Caption = "&Debug Mode is On" Then
            ...
            Let Ini.Application = "Log Errors"
            Let Ini.Parameter = Date$ & " " & Time$
            Let Ini.Value = "Programming Steam Screen, Button Close, stack is empty, underflow."
        End If
    End If
' -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
' End Subroutine
...
' -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    For TemporaryCounter = 0 To 9
        ...
        ' ---------------------------------------------------------------------------------------------------------------------------------------------------------------------
            Let Ini.Value = "Programming Steam Screen"
            Let TemporaryCounter = 11
        End If
    ' -------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
' ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    If TemporaryCounter = 10 Then
        If MainScreen!menuDebugMode.Caption = "&Debug Mode is On" Then
            ...
            Let Ini.Application = "Log Errors"
            Let Ini.Parameter = Date$ & " " & Time$
            Let Ini.Value = "Programming Steam Screen, Form Activate, stack is full, overflow."
        End If
    End If
...
' -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    If TemporaryBackgroundImage = "On" Then
        ...
        Else
            If MainScreen!menuDebugMode.Caption = "&Debug Mode is On" Then
                ...
                Let Ini.Application = "Log Errors"
                Let Ini.Parameter = Date$ & " " & Time$
                Let Ini.Value = "Programming Steam Screen, Form Activate, variable error in ATC.INI file for 'Transparency' setting."
            End If
        End If
    ...
    Else
        If MainScreen!menuDebugMode.Caption = "&Debug Mode is On" Then
            ...
            Let Ini.Application = "Log Errors"
            Let Ini.Parameter = Date$ & " " & Time$
            Let Ini.Value = "Programming Steam Screen, Form Activate, variable error in ATC.INI file for 'Background' setting."
        End If
    End If
...
' -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    Let Ini.Filename = App.Path$ & "\Atc.ini"
    Let Ini.Application = "Programming Steam Screen"
    Let Ini.Parameter = "Top"
    Let Ini.Value = Str$(ProgrammingSteam.Top)
    Let Ini.Parameter = "Left"
    Let Ini.Value = Str$(ProgrammingSteam.Left)
    Let Ini.Parameter = "Width"
    Let Ini.Value = Str(ProgrammingSteam.Width)
    Let Ini.Parameter = "Height"
    Let Ini.Value = Str(ProgrammingSteam.Height)
...
' -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    If TemporaryBackgroundImage = "On" Then
        ...
        Else
            If MainScreen!menuDebugMode.Caption = "&Debug Mode is On" Then
                ...
                Let Ini.Application = "Log Errors"
                Let Ini.Parameter = Date$ & " " & Time$
                Let Ini.Value = "Programming Steam Screen, Form Deactivate, variable error in ATC.INI file for 'Transparency' setting."
            End If
        End If
    ElseIf TemporaryBackgroundImage = "Off" Then
        AlphaBlend.Enabled = False
    Else
        If MainScreen!menuDebugMode.Caption = "&Debug Mode is On" Then
            ...
            Let Ini.Application = "Log Errors"
            Let Ini.Parameter = Date$ & " " & Time$
            Let Ini.Value = "Programming Seam Screen, Form Deactivate, variable error in ATC.INI file for 'Background' setting."
        End If
    End If
...
' Hide Screen
' -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ProgrammingSteam.Hide
    'unload Programmingsteam
...
'--------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    If Val(TemporaryValueLeft) = 0 And Val(TemporaryValueTop) = 0 Then
        ProgrammingSteam.Left = (Screen.Width - Width) / 2
        ProgrammingSteam.Top = (Screen.Height - Height) / 2
    Else
        If Val(TemporaryValueLeft) + ProgrammingSteam.Width > Screen.Width Then
            Let ProgrammingSteam.Left = Screen.Width - ProgrammingSteam.Width
        Else
            Let ProgrammingSteam.Left = Val(TemporaryValueLeft)
        End If
        If Val(TemporaryValueTop) + ProgrammingSteam.Height > Screen.Height Then
            Let ProgrammingSteam.Top = Screen.Height - ProgrammingSteam.Height
        Else
            Let ProgrammingSteam.Top = Val(TemporaryValueTop)
        End If
    End If
'--------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
Private Sub Form_Resize()
    ...
    If ProgrammingSteam.WindowState = vbMinimized Then
    ...
    ElseIf ProgrammingSteam.WindowState = vbNormal Then
