VERSION 4.00
Begin VB.Form MainlineSteam 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Automatic Train Control - Mainline Mode -Steam Database"
   ClientHeight    =   2445
   ClientLeft      =   2910
   ClientTop       =   8760
   ClientWidth     =   6690
   Height          =   2850
   Icon            =   "MainlineSteam.frx":0000
   Left            =   2850
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   ScaleHeight     =   2445
   ...
   Begin VB.PictureBox Picture1 
      ...
      Height          =   480
      Left            =   120
      Picture         =   "MainlineSteam.frx":0442
      ScaleHeight     =   480
      ScaleWidth      =   480
      TabIndex        =   1
      TabStop         =   0   'False
   ...
   Begin VB.Label Label1 
      Caption         =   "To enter in your steam locomotive data. I don't know alot about steam, so if you could help in developing this screen, please."
      Height          =   615
      Left            =   840
      TabIndex        =   2
      Top             =   120
   ...
   End
End
Attribute VB_Name = "MainlineSteam"
...
' -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    For TemporaryCounter = 9 To 0 Step -1
        ...
        If TemporaryScreen <> "Unused" Then
            If TemporaryScreen = "Mainline Steam Screen" Then
            ...
            ElseIf MainScreen!menuDebugMode.Caption = "&Debug Mode is On" Then
                ...
                Let Ini.Value = "Mainline Steam Screen, Button Close, current window is not listed in the stack to remove it and hide."
            End If
    ...
    ' -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
            ...
            ElseIf MainScreen!menuDebugMode.Caption = "&Debug Mode is On" Then
                ...
                Let Ini.Application = "Log Errors"
                Let Ini.Parameter = Date$ & " " & Time$
                Let Ini.Value = "Mainline Steam Screen, Button Close, trying to display the previous window using the screen stack, window not recognized."
            End If
' -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
' End Loop Premature
' -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
' -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    If TemporaryCounter = -1 Then
        If MainScreen!menuDebugMode.Caption = "&Debug Mode is On" Then
            ...
            Let Ini.Application = "Log Errors"
            Let Ini.Parameter = Date$ & " " & Time$
            Let Ini.Value = "Mainline Steam Screen, Button Close, stack is empty, underflow."
        End If
    End If
' -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
' End Subroutine
...
' =============================================================================================================================================================================
    ...
    For TemporaryCounter = 0 To 9
        ...
        ' ---------------------------------------------------------------------------------------------------------------------------------------------------------------------
            Let Ini.Value = "Mainline Steam Screen"
            Let TemporaryCounter = 11
        End If
    ' -------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
' ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    If TemporaryCounter = 10 Then
        If MainScreen!menuDebugMode.Caption = "&Debug Mode is On" Then
            ...
            Let Ini.Application = "Log Errors"
            Let Ini.Parameter = Date$ & " " & Time$
            Let Ini.Value = "Mainline Steam Screen, Form Activate, stack is full, overflow."
        End If
    End If
...
' -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    If TemporaryBackgroundImage = "On" Then
        ...
        Else
            If MainScreen!menuDebugMode.Caption = "&Debug Mode is On" Then
                ...
                Let Ini.Application = "Log Errors"
                Let Ini.Parameter = Date$ & " " & Time$
                Let Ini.Value = "Mainline Steam, Form Activate, variable error in ATC.INI file for 'Transparency' setting."
            End If
        End If
    ...
    Else
        If MainScreen!menuDebugMode.Caption = "&Debug Mode is On" Then
            ...
            Let Ini.Application = "Log Errors"
            Let Ini.Parameter = Date$ & " " & Time$
            Let Ini.Value = "Mainline Steam, Form Activate, variable error in ATC.INI file for 'Background' setting."
        End If
    End If
...
' -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    Let Ini.Filename = App.Path$ & "\Atc.ini"
    Let Ini.Application = "Mainline Steam"
    Let Ini.Parameter = "Top"
    Let Ini.Value = Str$(MainlineSteam.Top)
    Let Ini.Parameter = "Left"
    Let Ini.Value = Str$(MainlineSteam.Left)
    Let Ini.Parameter = "Width"
    Let Ini.Value = Str(MainlineSteam.Width)
    Let Ini.Parameter = "Height"
    Let Ini.Value = Str(MainlineSteam.Height)
...
' -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    If TemporaryBackgroundImage = "On" Then
        ...
        Else
            If MainScreen!menuDebugMode.Caption = "&Debug Mode is On" Then
                ...
                Let Ini.Application = "Log Errors"
                Let Ini.Parameter = Date$ & " " & Time$
                Let Ini.Value = "Mainline Steam, Form Deactivate, variable error in ATC.INI file for 'Transparency' setting."
            End If
        End If
    ElseIf TemporaryBackgroundImage = "Off" Then
        AlphaBlend.Enabled = False
    Else
        If MainScreen!menuDebugMode.Caption = "&Debug Mode is On" Then
            ...
            Let Ini.Application = "Log Errors"
            Let Ini.Parameter = Date$ & " " & Time$
            Let Ini.Value = "Mainline Steam, Form Deactivate, variable error in ATC.INI file for 'Background' setting."
        End If
    End If
...
' -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    Let Ini.Filename = App.Path$ & "\Atc.ini"
    Let Ini.Application = "Mainline Steam Screen"
    Let Ini.Parameter = "Top"
...
'--------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    If Val(TemporaryValueLeft) = 0 And Val(TemporaryValueTop) = 0 Then
        MainlineSteam.Left = (Screen.Width - Width) / 2
        MainlineSteam.Top = (Screen.Height - Height) / 2
    Else
        If Val(TemporaryValueLeft) + MainlineSteam.Width > Screen.Width Then
            Let MainlineSteam.Left = Screen.Width - MainlineSteam.Width
        Else
            Let MainlineSteam.Left = Val(TemporaryValueLeft)
        End If
        If Val(TemporaryValueTop) + MainlineSteam.Height > Screen.Height Then
            Let MainlineSteam.Top = Screen.Height - MainlineSteam.Height
        Else
            Let MainlineSteam.Top = Val(TemporaryValueTop)
        End If
    End If
'--------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
Private Sub Form_Resize()
    ...
    If MainlineSteam.WindowState = vbMinimized Then
    ...
    ElseIf MainlineSteam.WindowState = vbNormal Then
