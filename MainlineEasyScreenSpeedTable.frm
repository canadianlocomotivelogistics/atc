VERSION 4.00
Begin VB.Form MainlineEasyScreenSpeedTable 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Autoamtic Train Control - Mainline Mode - Easy Screen Speed Table"
   ClientHeight    =   5355
   ClientLeft      =   2145
   ClientTop       =   4215
   ClientWidth     =   7140
   FillStyle       =   0  'Solid
   Height          =   5760
   Icon            =   "MainlineEasyScreenSpeedTable.frx":0000
   Left            =   2085
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   ScaleHeight     =   5355
   ...
   Begin VB.PictureBox Picture1 
      ...
      Height          =   480
      Left            =   120
      Picture         =   "MainlineEasyScreenSpeedTable.frx":0442
      ScaleHeight     =   480
      ScaleWidth      =   480
      TabIndex        =   0
      Top             =   120
   ...
   Begin VB.Label Label1 
      Caption         =   $"MainlineEasyScreenSpeedTable.frx":0884
      Height          =   615
      Left            =   840
      TabIndex        =   1
      Top             =   120
   ...
   End
End
Attribute VB_Name = "MainlineEasyScreenSpeedTable"
...
Private Sub ButtonClose_Click()
    ...
    For t = 67 To 94
        Let MainlineSpeedTable.LocomotiveDecoderCVd(t).Text = SliderSpeedTableCV(t).Value
    Next t
' -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
' -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    For TemporaryCounter = 9 To 0 Step -1
        ...
        If TemporaryScreen <> "Unused" Then
            If TemporaryScreen = "Mainline Easy Screen Speed Table Screen" Then
            ...
            ElseIf MainScreen!menuDebugMode.Caption = "&Debug Mode is On" Then
                ...
                Let Ini.Value = "Mainline Easy Screen Speed Table Screen, Button Close, current window is not listed in the stack to remove it and hide."
            End If
    ...
    ' -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
            ...
            ElseIf MainScreen!menuDebugMode.Caption = "&Debug Mode is On" Then
                ...
                Let Ini.Application = "Log Errors"
                Let Ini.Parameter = Date$ & " " & Time$
                Let Ini.Value = "Mainline Easy Screen Speed Table Screen, Button Close, trying to display the previous window using the screen stack, window not recognized."
            End If
' -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
' End Loop Premature
' -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
' -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    If TemporaryCounter = -1 Then
        If MainScreen!menuDebugMode.Caption = "&Debug Mode is On" Then
            ...
            Let Ini.Application = "Log Errors"
            Let Ini.Parameter = Date$ & " " & Time$
            Let Ini.Value = "Mainline East SCreen Speed Table Screen, Button Close, stack is empty, underflow."
        End If
    End If
' -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
' End Subroutine
...
Private Sub ButtonPrint_Click()
    ...
    MainlineEasyScreenSpeedTable.PrintForm
...
' =============================================================================================================================================================================
    ...
    For TemporaryCounter = 0 To 9
        ...
        ' ---------------------------------------------------------------------------------------------------------------------------------------------------------------------
            Let Ini.Value = "Mainline Easy Screen Speed Table Screen"
            Let TemporaryCounter = 11
        End If
    ' -------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
' ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    If TemporaryCounter = 10 Then
        If MainScreen!menuDebugMode.Caption = "&Debug Mode is On" Then
            ...
            Let Ini.Application = "Log Errors"
            Let Ini.Parameter = Date$ & " " & Time$
            Let Ini.Value = "Mainline Easy Screen Speed Table Screen, Form Activate, stack is full, overflow."
        End If
    End If
...
' -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    If TemporaryBackgroundImage = "On" Then
        ...
        Else
            If MainScreen!menuDebugMode.Caption = "&Debug Mode is On" Then
                ...
                Let Ini.Application = "Log Errors"
                Let Ini.Parameter = Date$ & " " & Time$
                Let Ini.Value = "Mainline Easy Screen Spped Table Screen, Form Activate, variable error in ATC.INI file for 'Transparency' setting."
            End If
        End If
    ...
    Else
        If MainScreen!menuDebugMode.Caption = "&Debug Mode is On" Then
            ...
            Let Ini.Application = "Log Errors"
            Let Ini.Parameter = Date$ & " " & Time$
            Let Ini.Value = "Mainline Easy Screen Speed Table Screen, Form Activate, variable error in ATC.INI file for 'Background' setting."
        End If
    End If
...
' -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    Let Ini.Filename = App.Path$ & "\Atc.ini"
    Let Ini.Application = "Mainline Easy Screen Speed Table Screen"
    Let Ini.Parameter = "Top"
    Let Ini.Value = Str$(MainlineEasyScreenSpeedTable.Top)
    Let Ini.Parameter = "Left"
    Let Ini.Value = Str$(MainlineEasyScreenSpeedTable.Left)
    Let Ini.Parameter = "Width"
    Let Ini.Value = Str(MainlineEasyScreenSpeedTable.Width)
    Let Ini.Parameter = "Height"
    Let Ini.Value = Str(MainlineEasyScreenSpeedTable.Height)
...
' -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    If TemporaryBackgroundImage = "On" Then
        ...
        Else
            If MainScreen!menuDebugMode.Caption = "&Debug Mode is On" Then
                ...
                Let Ini.Application = "Log Errors"
                Let Ini.Parameter = Date$ & " " & Time$
                Let Ini.Value = "Mainline Easy Screen Speed Table Screen, Form Deactivate, variable error in ATC.INI file for 'Transparency' setting."
            End If
        End If
    ElseIf TemporaryBackgroundImage = "Off" Then
        AlphaBlend.Enabled = False
    Else
        If MainScreen!menuDebugMode.Caption = "&Debug Mode is On" Then
            ...
            Let Ini.Application = "Log Errors"
            Let Ini.Parameter = Date$ & " " & Time$
            Let Ini.Value = "Mainline Easy Screen Speed Table Screen, Form Deactivate, variable error in ATC.INI file for 'Background' setting."
        End If
    End If
...
' Hide Screen
' -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    MainlineEasyScreenSpeedTable.Hide
    'unload Mainlineeasyscreenspeedtable
...
'--------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    If Val(TemporaryValueLeft) = 0 And Val(TemporaryValueTop) = 0 Then
        MainlineEasyScreenSpeedTable.Left = (Screen.Width - Width) / 2
        MainlineEasyScreenSpeedTable.Top = (Screen.Height - Height) / 2
    Else
        If Val(TemporaryValueLeft) + MainlineEasyScreenSpeedTable.Width > Screen.Width Then
            Let MainlineEasyScreenSpeedTable.Left = Screen.Width - MainlineEasyScreenSpeedTable.Width
        Else
            Let MainlineEasyScreenSpeedTable.Left = Val(TemporaryValueLeft)
        End If
        If Val(TemporaryValueTop) + MainlineEasyScreenSpeedTable.Height > Screen.Height Then
            Let MainlineEasyScreenSpeedTable.Top = Screen.Height - MainlineEasyScreenSpeedTable.Height
        Else
            Let MainlineEasyScreenSpeedTable.Top = Val(TemporaryValueTop)
        End If
    End If
'--------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
' -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    If MainScreen.menuBalloonHelp.Caption = "&Balloon Help is On" Then
        ...
        Let TemporaryText1 = "This button closes this window and returns you" & vbCrLf & "to the 'Mainline Mode - Speed Table'. All values from" & vbCrLf & "this window will be saved to the 'Mainline Mode - Speed Table' screen."
...
' -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    For TemporaryVariableT = 67 To 94
        Let SliderSpeedTableCV(TemporaryVariableT).Value = Val(MainlineSpeedTable.LocomotiveDecoderCVd(TemporaryVariableT).Text)
...
Private Sub Form_Resize()
    ...
    If MainlineEasyScreenSpeedTable.WindowState = vbMinimized Then
    ...
    ElseIf MainlineEasyScreenSpeedTable.WindowState = vbNormal Then
...
'
    ...
    Let Ini.Filename = App.Path$ & "\Atc.ini"
    Let Ini.Application = "Mainline Easy Screen Speed Table"
    Let Ini.Parameter = "Top"
    Let Ini.Value = Str$(MainlineEasyScreenSpeedTable.Top)
    Let Ini.Parameter = "Left"
    Let Ini.Value = Str$(MainlineEasyScreenSpeedTable.Left)
