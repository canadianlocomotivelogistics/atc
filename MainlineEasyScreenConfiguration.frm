VERSION 4.00
Begin VB.Form MainlineEasyScreenConfiguration 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Automatic Train Control - Mainline Mode - Easy Screen - Configuration CV29"
   ClientHeight    =   4095
   ClientLeft      =   2265
   ClientTop       =   2670
   ClientWidth     =   7065
   Height          =   4500
   Icon            =   "MainlineEasyScreenConfiguration.frx":0000
   Left            =   2205
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   ScaleHeight     =   4095
   ...
   Begin VB.Label Label5 
      Caption         =   $"MainlineEasyScreenConfiguration.frx":0442
      Height          =   375
      Left            =   840
      TabIndex        =   12
      Top             =   120
   ...
   Begin VB.Image Image1 
      Height          =   480
      Left            =   120
      Picture         =   "MainlineEasyScreenConfiguration.frx":04CF
      Top             =   120
      Width           =   480
   End
   Begin VB.Line Line2 
   ...
   End
End
Attribute VB_Name = "MainlineEasyScreenConfiguration"
Attribute VB_Creatable = False
Attribute VB_Exposed = False

Private Sub ButtonClose_Click()

    Let MainlineDecoder!LocomotiveDecoderCVd(29).Text = textboxcvvalue(29).Text
' -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
' -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    For TemporaryCounter = 9 To 0 Step -1
        ...
        If TemporaryScreen <> "Unused" Then
            If TemporaryScreen = "Mainline Easy Screen Configuration Screen" Then
            ...
            ElseIf MainScreen!menuDebugMode.Caption = "&Debug Mode is On" Then
                ...
                Let Ini.Value = "Mainline Easy Screen Configuration Screen, Button Close, current window is not listed in the stack to remove it and hide."
            End If
    ...
    ' -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
            ...
            ElseIf MainScreen!menuDebugMode.Caption = "&Debug Mode is On" Then
                ...
                Let Ini.Application = "Log Errors"
                Let Ini.Parameter = Date$ & " " & Time$
                Let Ini.Value = "Mainline Easy Screen Configuraton Screen, Button Close, trying to display the previous window using the screen stack, window not recognized."
            End If
' -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
' End Loop Premature
' -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
' -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    If TemporaryCounter = -1 Then
        If MainScreen!menuDebugMode.Caption = "&Debug Mode is On" Then
            ...
            Let Ini.Application = "Log Errors"
            Let Ini.Parameter = Date$ & " " & Time$
            Let Ini.Value = "Mainline Easy Screen Configuration Screen, Button Close, stack is empty, underflow."
        End If
    End If
' -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
' End Subroutine
...
Private Sub ButtonPrint_Click()
    ...
    MainlineEasyScreenConfiguration.PrintForm
...
' =============================================================================================================================================================================
    ...
    For TemporaryCounter = 0 To 9
        ...
        ' ---------------------------------------------------------------------------------------------------------------------------------------------------------------------
            Let Ini.Value = "Mainline Easy Screen Configuration Screen"
            Let TemporaryCounter = 11
        End If
    ' -------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
' ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    If TemporaryCounter = 10 Then
        If MainScreen!menuDebugMode.Caption = "&Debug Mode is On" Then
            ...
            Let Ini.Application = "Log Errors"
            Let Ini.Parameter = Date$ & " " & Time$
            Let Ini.Value = "Mainline Easy Screen Configuraton Screen, Form Activate, stack is full, overflow."
        End If
    End If
...
' -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    If TemporaryBackgroundImage = "On" Then
        ...
        Else
            If MainScreen!menuDebugMode.Caption = "&Debug Mode is On" Then
                ...
                Let Ini.Application = "Log Errors"
                Let Ini.Parameter = Date$ & " " & Time$
                Let Ini.Value = "Mainline Easy Screen Configuration Screen, Form Activate, variable error in ATC.INI file for 'Transparency' setting."
            End If
        End If
    ...
    Else
        If MainScreen!menuDebugMode.Caption = "&Debug Mode is On" Then
            ...
            Let Ini.Application = "Log Errors"
            Let Ini.Parameter = Date$ & " " & Time$
            Let Ini.Value = "Mainline Easy Screen Configuration Screen, Form Activate, variable error in ATC.INI file for 'Background' setting."
        End If
    End If
...
' -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    Let Ini.Filename = App.Path$ & "\Atc.ini"
    Let Ini.Application = "Mainline Easy Screen Configuration Screen"
    Let Ini.Parameter = "Top"
    Let Ini.Value = Str$(MainlineEasyScreenConfiguration.Top)
    Let Ini.Parameter = "Left"
    Let Ini.Value = Str$(MainlineEasyScreenConfiguration.Left)
    Let Ini.Parameter = "Width"
    Let Ini.Value = Str(MainlineEasyScreenConfiguration.Width)
    Let Ini.Parameter = "Height"
    Let Ini.Value = Str(MainlineEasyScreenConfiguration.Height)
...
' -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    If TemporaryBackgroundImage = "On" Then
        ...
        Else
            If MainScreen!menuDebugMode.Caption = "&Debug Mode is On" Then
                ...
                Let Ini.Application = "Log Errors"
                Let Ini.Parameter = Date$ & " " & Time$
                Let Ini.Value = "Mainline Easy Screen Configuration Screen, Form Deactivate, variable error in ATC.INI file for 'Transparency' setting."
            End If
        End If
    ElseIf TemporaryBackgroundImage = "Off" Then
        AlphaBlend.Enabled = False
    Else
        If MainScreen!menuDebugMode.Caption = "&Debug Mode is On" Then
            ...
            Let Ini.Application = "Log Errors"
            Let Ini.Parameter = Date$ & " " & Time$
            Let Ini.Value = "Mainline Easy Screen Configuration Screen, Form Deactivate, variable error in ATC.INI file for 'Background' setting."
        End If
    End If
...
' Hide Screen
' -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    MainlineEasyScreenConfiguration.Hide
    'unload Mainlineeasyscreenconfiguration
...
' -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    Let Ini.Filename = App.Path$ & "\Atc.ini"
    Let Ini.Application = "Mainline Easy Screen Configuration"
    Let Ini.Parameter = "Top"
...
'--------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    If Val(TemporaryValueLeft) = 0 And Val(TemporaryValueTop) = 0 Then
        MainlineEasyScreenConfiguration.Left = (Screen.Width - Width) / 2
        MainlineEasyScreenConfiguration.Top = (Screen.Height - Height) / 2
    Else
        If Val(TemporaryValueLeft) + MainlineEasyScreenConfiguration.Width > Screen.Width Then
            Let MainlineEasyScreenConfiguration.Left = Screen.Width - MainlineEasyScreenConfiguration.Width
        Else
            Let MainlineEasyScreenConfiguration.Left = Val(TemporaryValueLeft)
        End If
        If Val(TemporaryValueTop) + MainlineEasyScreenConfiguration.Height > Screen.Height Then
            Let MainlineEasyScreenConfiguration.Top = Screen.Height - MainlineEasyScreenConfiguration.Height
        Else
            Let MainlineEasyScreenConfiguration.Top = Val(TemporaryValueTop)
        End If
    End If
'--------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
' -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    If MainScreen.menuBalloonHelp.Caption = "&Balloon Help is On" Then
        ...
        Let TemporaryText1 = "This button closes the window (Mainline Easy" & vbCrLf & "Screen Configuration)and returns you to " & vbCrLf & "Mainline Decoder Screen."
...
Let temp = Val(MainlineDecoder!LocomotiveDecoderCVd(29).Text)
...
Private Sub Form_Resize()
    ...
    If MainlineEasyScreenConfiguration.WindowState = vbMinimized Then
    ...
    ElseIf MainlineEasyScreenConfiguration.WindowState = vbNormal Then
...
'
    ...
    Let Ini.Filename = App.Path$ & "\Atc.ini"
    Let Ini.Application = "Mainline Easy Screen Configuraton"
    Let Ini.Parameter = "Top"
    Let Ini.Value = Str$(MainlineEasyScreenConfiguration.Top)
    Let Ini.Parameter = "Left"
    Let Ini.Value = Str$(MainlineEasyScreenConfiguration.Left)
