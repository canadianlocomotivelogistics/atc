VERSION 4.00
Begin VB.Form MainlineRollingStock 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Automatic Train Control - Mainline Mode - Rolling Stock"
   ClientHeight    =   2505
   ClientLeft      =   435
   ClientTop       =   4980
   ClientWidth     =   6600
   Height          =   2910
   Icon            =   "MainlineRollingStock.frx":0000
   Left            =   375
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   ScaleHeight     =   2505
   ...
   Begin VB.PictureBox Picture1 
      ...
      Height          =   480
      Left            =   120
      Picture         =   "MainlineRollingStock.frx":0442
      ScaleHeight     =   480
      ScaleWidth      =   480
      TabIndex        =   1
      TabStop         =   0   'False
   ...
   Begin VB.Label Label1 
      Caption         =   "Enter in your rolling stock, have weigh bills, or other orders for your engineers and conductors."
      Height          =   495
      Left            =   840
      TabIndex        =   2
      Top             =   120
   ...
   End
End
Attribute VB_Name = "MainlineRollingStock"
...
' -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    For TemporaryCounter = 9 To 0 Step -1
        ...
        If TemporaryScreen <> "Unused" Then
            If TemporaryScreen = "Mainline Rolling Stock Screen" Then
            ...
            ElseIf MainScreen!menuDebugMode.Caption = "&Debug Mode is On" Then
                ...
                Let Ini.Value = "Mainline Rolling Stock Screen, Button Close, current window is not listed in the stack to remove it and hide."
            End If
    ...
    ' -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
            ...
            ElseIf MainScreen!menuDebugMode.Caption = "&Debug Mode is On" Then
                ...
                Let Ini.Application = "Log Errors"
                Let Ini.Parameter = Date$ & " " & Time$
                Let Ini.Value = "Mainline Rolling Stock Screen, Button Close, trying to display the previous window using the screen stack, window not recognized."
            End If
' -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
' End Loop Premature
' -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
' -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    If TemporaryCounter = -1 Then
        If MainScreen!menuDebugMode.Caption = "&Debug Mode is On" Then
            ...
            Let Ini.Application = "Log Errors"
            Let Ini.Parameter = Date$ & " " & Time$
            Let Ini.Value = "Mainline Rolling Stock Screen, Button Close, stack is empty, underflow."
        End If
    End If
' -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
' End Subroutine
...
Private Sub ButtonPrint_Click()
    ...
    MainlineRollingStock.PrintForm
...
' =============================================================================================================================================================================
    ...
    For TemporaryCounter = 0 To 9
        ...
        ' ---------------------------------------------------------------------------------------------------------------------------------------------------------------------
            Let Ini.Value = "Mainline Rolling Stock Screen"
            Let TemporaryCounter = 11
        End If
    ' -------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
' ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    If TemporaryCounter = 10 Then
        If MainScreen!menuDebugMode.Caption = "&Debug Mode is On" Then
            ...
            Let Ini.Application = "Log Errors"
            Let Ini.Parameter = Date$ & " " & Time$
            Let Ini.Value = "Mainline Rolling Stock Screen, Form Activate, stack is full, overflow."
        End If
    End If
' -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
' -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    If TemporaryBackgroundImage = "On" Then
        ...
        Else
            If MainScreen!menuDebugMode.Caption = "&Debug Mode is On" Then
                ...
                Let Ini.Application = "Log Errors"
                Let Ini.Parameter = Date$ & " " & Time$
                Let Ini.Value = "Mainline Rolling Stock Screen, Form Activate, variable error in ATC.INI file for 'Transparency' setting."
            End If
        End If
    ...
    Else
        If MainScreen!menuDebugMode.Caption = "&Debug Mode is On" Then
            ...
            Let Ini.Application = "Log Errors"
            Let Ini.Parameter = Date$ & " " & Time$
            Let Ini.Value = "Mainline Rolling Stock Screen, Form Activate, variable error in ATC.INI file for 'Background' setting."
        End If
    End If
...
' -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    Let Ini.Filename = App.Path$ & "\Atc.ini"
    Let Ini.Application = "Mainline Rolling Stock Screen"
    Let Ini.Parameter = "Top"
    Let Ini.Value = Str$(MainlineRollingStock.Top)
    Let Ini.Parameter = "Left"
    Let Ini.Value = Str$(MainlineRollingStock.Left)
    Let Ini.Parameter = "Width"
    Let Ini.Value = Str(MainlineRollingStock.Width)
    Let Ini.Parameter = "Height"
    Let Ini.Value = Str(MainlineRollingStock.Height)
...
' -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    If TemporaryBackgroundImage = "On" Then
        ...
        Else
            If MainScreen!menuDebugMode.Caption = "&Debug Mode is On" Then
                ...
                Let Ini.Application = "Log Errors"
                Let Ini.Parameter = Date$ & " " & Time$
                Let Ini.Value = "Mainline Rolling Stock Screen, Form Deactivate, variable error in ATC.INI file for 'Transparency' setting."
            End If
        End If
    ElseIf TemporaryBackgroundImage = "Off" Then
        AlphaBlend.Enabled = False
    Else
        If MainScreen!menuDebugMode.Caption = "&Debug Mode is On" Then
            ...
            Let Ini.Application = "Log Errors"
            Let Ini.Parameter = Date$ & " " & Time$
            Let Ini.Value = "Mainline Rolling Stock Screen, Form Deactivate, variable error in ATC.INI file for 'Background' setting."
        End If
    End If
...
' Hide Screen
' -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    MainlineRollingStock.Hide
    'unload Mainlinerollingstock
...
'--------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    If Val(TemporaryValueLeft) = 0 And Val(TemporaryValueTop) = 0 Then
        MainlineRollingStock.Left = (Screen.Width - Width) / 2
        MainlineRollingStock.Top = (Screen.Height - Height) / 2
    Else
        If Val(TemporaryValueLeft) + MainlineRollingStock.Width > Screen.Width Then
            Let MainlineRollingStock.Left = Screen.Width - MainlineRollingStock.Width
        Else
            Let MainlineRollingStock.Left = Val(TemporaryValueLeft)
        End If
        If Val(TemporaryValueTop) + MainlineRollingStock.Height > Screen.Height Then
            Let MainlineRollingStock.Top = Screen.Height - MainlineRollingStock.Height
        Else
            Let MainlineRollingStock.Top = Val(TemporaryValueTop)
        End If
    End If
'--------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
Private Sub Form_Resize()
    ...
    If MainlineRollingStock.WindowState = vbMinimized Then
    ...
    ElseIf MainlineRollingStock.WindowState = vbNormal Then
