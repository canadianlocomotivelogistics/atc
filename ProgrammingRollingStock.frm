VERSION 4.00
Begin VB.Form ProgrammingRollingStock 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Automatic Train Control - Programming Mode -Rolling Stock Programming Database"
   ClientHeight    =   2160
   ClientLeft      =   1305
   ClientTop       =   7650
   ClientWidth     =   6690
   Height          =   2565
   Icon            =   "ProgrammingRollingStock.frx":0000
   Left            =   1245
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   ScaleHeight     =   2160
   ...
   Begin VB.PictureBox PictureIcon 
      ...
      Height          =   480
      Left            =   120
      Picture         =   "ProgrammingRollingStock.frx":0442
      ScaleHeight     =   480
      ScaleWidth      =   480
      TabIndex        =   1
      TabStop         =   0   'False
   ...
   Begin VB.Label Label1 
      Caption         =   $"ProgrammingRollingStock.frx":0884
      Height          =   735
      Left            =   720
      TabIndex        =   2
      Top             =   120
   ...
   End
End
Attribute VB_Name = "ProgrammingRollingStock"
...
Private Sub Command1_Click()
    ...
    ProgrammingRollingStock.PrintForm
...
' -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    For TemporaryCounter = 9 To 0 Step -1
        ...
        If TemporaryScreen <> "Unused" Then
            If TemporaryScreen = "Programming Rolling Stock Screen" Then
            ...
            ElseIf MainScreen!menuDebugMode.Caption = "&Debug Mode is On" Then
                ...
                Let Ini.Value = "Programming Rolling Stock Screen, Button Close, current window is not listed in the stack to remove it and hide."
            End If
    ...
    ' -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
            ...
            ElseIf MainScreen!menuDebugMode.Caption = "&Debug Mode is On" Then
                ...
                Let Ini.Application = "Log Errors"
                Let Ini.Parameter = Date$ & " " & Time$
                Let Ini.Value = "Programming Rolling Stock Screen, Button Close, trying to display the previous window using the screen stack, window not recognized."
            End If
' -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
' End Loop Premature
' -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
' -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    If TemporaryCounter = -1 Then
        If MainScreen!menuDebugMode.Caption = "&Debug Mode is On" Then
            ...
            Let Ini.Application = "Log Errors"
            Let Ini.Parameter = Date$ & " " & Time$
            Let Ini.Value = "Programming Rolling Stock Screen, Button Close, stack is empty, underflow."
        End If
    End If
' -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
' End Subroutine
...
' -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    For TemporaryCounter = 0 To 9
        ...
        ' ---------------------------------------------------------------------------------------------------------------------------------------------------------------------
            Let Ini.Value = "Programming Rolling Stock Screen"
            Let TemporaryCounter = 11
        End If
    ' -------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
' ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    If TemporaryCounter = 10 Then
        If MainScreen!menuDebugMode.Caption = "&Debug Mode is On" Then
            ...
            Let Ini.Application = "Log Errors"
            Let Ini.Parameter = Date$ & " " & Time$
            Let Ini.Value = "Programming Rolling Stock Screen, Form Activate, stack is full, overflow."
        End If
    End If
...
' -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    If TemporaryBackgroundImage = "On" Then
        ...
        Else
            If MainScreen!menuDebugMode.Caption = "&Debug Mode is On" Then
                ...
                Let Ini.Application = "Log Errors"
                Let Ini.Parameter = Date$ & " " & Time$
                Let Ini.Value = "Programming Rolling Stock Screen, Form Activate, variable error in ATC.INI file for 'Transparency' setting."
            End If
        End If
    ...
    Else
        If MainScreen!menuDebugMode.Caption = "&Debug Mode is On" Then
            ...
            Let Ini.Application = "Log Errors"
            Let Ini.Parameter = Date$ & " " & Time$
            Let Ini.Value = "Programming Rolling Stock Screen, Form Activate, variable error in ATC.INI file for 'Background' setting."
        End If
    End If
...
' -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    Let Ini.Filename = App.Path$ & "\Atc.ini"
    Let Ini.Application = "Programming Rolling Stock Screen"
    Let Ini.Parameter = "Top"
    Let Ini.Value = Str$(ProgrammingRollingStock.Top)
    Let Ini.Parameter = "Left"
    Let Ini.Value = Str$(ProgrammingRollingStock.Left)
    Let Ini.Parameter = "Width"
    Let Ini.Value = Str(ProgrammingRollingStock.Width)
    Let Ini.Parameter = "Height"
    Let Ini.Value = Str(ProgrammingRollingStock.Height)
...
' -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    If TemporaryBackgroundImage = "On" Then
        ...
        Else
            If MainScreen!menuDebugMode.Caption = "&Debug Mode is On" Then
                ...
                Let Ini.Application = "Log Errors"
                Let Ini.Parameter = Date$ & " " & Time$
                Let Ini.Value = "Programming Rolling Stock Screen, Form Deactivate, variable error in ATC.INI file for 'Transparency' setting."
            End If
        End If
    ElseIf TemporaryBackgroundImage = "Off" Then
        AlphaBlend.Enabled = False
    Else
        If MainScreen!menuDebugMode.Caption = "&Debug Mode is On" Then
            ...
            Let Ini.Application = "Log Errors"
            Let Ini.Parameter = Date$ & " " & Time$
            Let Ini.Value = "Programming Rolling Stock Screen, Form Deactivate, variable error in ATC.INI file for 'Background' setting."
        End If
    End If
...
' Hide Screen
' -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ProgrammingRollingStock.Hide
    'unload Programmingrollingstock
...
'--------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    If Val(TemporaryValueLeft) = 0 And Val(TemporaryValueTop) = 0 Then
        ProgrammingRollingStock.Left = (Screen.Width - Width) / 2
        ProgrammingRollingStock.Top = (Screen.Height - Height) / 2
    Else
        If Val(TemporaryValueLeft) + ProgrammingRollingStock.Width > Screen.Width Then
            Let ProgrammingRollingStock.Left = Screen.Width - ProgrammingRollingStock.Width
        Else
            Let ProgrammingRollingStock.Left = Val(TemporaryValueLeft)
        End If
        If Val(TemporaryValueTop) + ProgrammingRollingStock.Height > Screen.Height Then
            Let ProgrammingRollingStock.Top = Screen.Height - ProgrammingRollingStock.Height
        Else
            Let ProgrammingRollingStock.Top = Val(TemporaryValueTop)
        End If
    End If
'--------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
Private Sub Form_Resize()
    ...
    If ProgrammingRollingStock.WindowState = vbMinimized Then
    ...
    ElseIf ProgrammingRollingStock.WindowState = vbNormal Then
